...
| D-004 | 2026-08-27 | Password reset in Phase 1 = admin-initiated. System Admin triggers reset → one-time token returned in API response / shown in admin UI, user sets new password. No SMTP. | Email is not in Phase 1 scope; PRS lists "password reset" as a capability without a delivery channel | maintainers | accepted |
