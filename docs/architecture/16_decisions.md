...
| D-019 | 2026-08-28 | Optional self-service "Forgot password": `POST /auth/password-reset/request {identifier}` behind an optional SMTP mailer (`Mailer` port, `SmtpMailer`/`NullMailer`). When SMTP is unset → response tells the user to contact an administrator. When set → a matched **non-admin active** account is emailed a reset artefact; non-matches get the same generic message (no existence disclosure). **System-admin accounts always get "contact the maintainer"** and are never emailed. New `POST /auth/password` lets a signed-in user change their own password. **Amended 2026-09-03 (D-020).** | User request; PRS §15 lists "password reset"; keeps email strictly optional so the zero-config self-hosted path is unchanged | maintainers | amended by D-020 |
| D-020 | 2026-09-03 | Self-service reset emails a **one-time link** (`/reset?token=…`, `password_reset_token` row, `hash_token`, 1-hour TTL, single-use, prior unused links invalidated) instead of a temporary password. The password is not changed and sessions are not revoked until the link is redeemed via `POST /auth/password-reset/complete`, so a failed email delivery can never lock a user out. Redemption revokes all of that user's sessions. | Code review finding #2: the old flow committed the password change + session revocation *before* attempting delivery and reported success even on SMTP failure | owner | accepted |
